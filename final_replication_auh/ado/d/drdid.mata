*! v1.1 Two new Methods RC2 and DRIMP2
*!              Also Addressed Bug with Clustered ST errors with RC
...
class drdid {
      ...
      void              reg_rc()
      // Reg2 is the Double reg
      void              reg2_rc() 
      void              drimp_rc()
      // This is Double Reg based on Drimp/
      void              drimp2_rc()
...
void drdid::msetup_panel(){
      ...
      // Basically If using other methods and there are no covariates, then 
      // OLS1 is the best option
      if (method_type != 4) {
                   if (kx ==0)  {
                         method_type=4 
                   }
 ...
 // Very Inneficient way to adress the problem. But may still be better than the alternative
                  method_type=4
      ...
      // I can probably get rid of ID and keep only OID for matching 
...
void drdid::msetup_rc(){
      ...
      //Doesnt need to be Redone. It comes from the data
      //oid 
      id=1::rows(id)      
      // keeps only those with 2 observations
      tmt  = tmt :==max(tmt)
      ...
      if (kx>0) {
            // If any variable. Check which variables have variation.
            // Does Gets rid of Constants.
            xvar = select(xvar, select4() )
            kx   = cols(xvar)
      }
      // If no variables left, using SIMPLE OLS 
      if (kx ==0)  {
            method_type=4 
      }
...
void drdid::drimp2_rc(){
 ...
 // This is similar to Statas hdidregress
void drdid::reg2_rc(){
    // main Loading variables
       real matrix w10, w11, w00, w01, w1
      // Avoid Weights that are Too large
...
void drdid::drdid(){
      ...
      if (data_type == 1) {
            ...
            // IF RC 6 methods. Method 6 may be redundant
            msetup_rc()
            ...
            else if (method_type ==6) drimp2_rc()
      } 
